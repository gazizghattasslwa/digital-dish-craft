  ...
  try {
    ...
    switch (action) {
      case 'add':
        ...
        return await addCustomDomain(supabaseClient, domain, restaurantId);
      case 'verify':
        if (!domain) throw new Error('Domain is required for verify action');
        return await verifyDomain(supabaseClient, domain);
      case 'delete':
        if (!domain) throw new Error('Domain is required for delete action');
        return await deleteCustomDomain(supabaseClient, domain, restaurantId);
      case 'fetch':
        return await fetchCustomDomains(supabaseClient, restaurantId);
      default:
...
async function fetchCustomDomains(supabaseClient: any, restaurantId: string) {
...
async function addCustomDomain(supabaseClient: any, domain: string, restaurantId: string) {
...
async function verifyDomain(supabaseClient: any, domain: string): Promise<Response> {
...
async function deleteCustomDomain(supabaseClient: any, domain: string, restaurantId: string) {
  const { error } = await supabaseClient
    .from('custom_domains')
    .delete()
    .eq('domain', domain.toLowerCase())
