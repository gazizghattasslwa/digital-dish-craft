      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div 
              key={index} 
              className="relative bg-card border border-border rounded-2xl p-8 hover:border-primary/20 transition-all duration-300"
            >
              {feature.badge && (
                <div className="absolute -top-2 -right-2 bg-accent text-foreground px-2 py-1 rounded-lg text-xs font-medium">
                  {feature.badge}
                </div>
              )}
        ...
        <div className="text-center mt-16">
          ...
          <button className="bg-accent text-foreground hover:bg-accent/90 px-8 py-3 rounded-xl font-medium transition-colors">
            Get Started Today
          </button>
        </div>
      </div>
